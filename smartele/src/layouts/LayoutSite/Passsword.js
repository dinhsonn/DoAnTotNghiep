  ...
  const resetPassword = async () => {
    try {
      if (newPassword.length < 10 || !/[a-zA-Z]/.test(newPassword)) {
        alert("Mật khẩu mới phải có ít nhất 10 ký tự và bao gồm cả chữ cái.");
        return;
      }
      ...
      const data = await response.json(); // Lấy phản hồi JSON từ server
      if (response.ok) {
        alert("Mật khẩu đã được đặt lại thành công.");
        navigate('/');
      } else {
        if (data.error === "Mã xác nhận không đúng") {
